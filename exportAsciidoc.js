...
function getPadAsciidoc(pad, revNum, callback)
{
  var atext = pad.atext;
  var Asciidoc;
  async.waterfall([
  ...
  // convert atext to Asciidoc
  function (callback)
  {
    Asciidoc = getAsciidocFromAtext(pad, atext);
    callback(null);
  }],
  ...
  {
    if(ERR(err, callback)) return;
    callback(null, Asciidoc);
  });
}

exports.getPadAsciidoc = getPadAsciidoc;

function getAsciidocFromAtext(pad, atext)
{
  var apool = pad.apool();
  ...
  function getLineAsciidoc(text, attribs)
  {
...
//    }
    ...
    assem = assem.replace(/\_/g, '\\_'); // this breaks Asciidoc math mode: $\sum_i^j$ becomes $\sum\_i^j$
  ...
  for (var i = 0; i < textLines.length; i++)
  {
    ...
    var lineContent = getLineAsciidoc(line.text, line.aline);
...
exports.getPadAsciidocDocument = function (padId, revNum, callback)
{
  padManager.getPad(padId, function (err, pad)
  {
    if(ERR(err, callback)) return;

    getPadAsciidoc(pad, revNum, function (err, Asciidoc)
    {
      if(ERR(err, callback)) return;
      callback(null, Asciidoc);
    });
